...
import type { PageData, GemResponses } from "~/types";

const pageData = useState<PageData | null>("page-data", () => null);
const responses = useState<GemResponses[]>("response-array", () => []);
...
const handleGemini = async (event: Event): Promise<void> => {
  const e = event as SubmitEvent;
  e.preventDefault();
  loader.value = true;
  if (!prompt.value || !pageData.value) {
...
<template>
  <div v-if="pageData">
    <div :class="`container mx-auto relative pt-40`">
      <h1 class="text-5xl text-white font-normal">{{ pageData[0]?.title }}</h1>
      <h2
        class="text-[13vw] leading-[12vw] text-rose-600 mb-40 font-bold capitalize"
      >
        {{ pageData[0]?.tagline }}
      </h2>
    ...
    <!-- <div class="fixed z-index-[1] inset-x-0 bottom-10 flex justify-center">
      ...
      >
        ...
        />
        <button type="submit" class="bg-blue-500 text-white p-2 mt-5">
          Submit
        </button>
      </form>
    </div> -->
